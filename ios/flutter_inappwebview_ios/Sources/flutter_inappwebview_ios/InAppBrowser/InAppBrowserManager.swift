...
import UIKit
import WebKit
import flutter_inappwebview_ios_resources
...
public class InAppBrowserManager: ChannelDelegate {
  ...
  public func presentViewController(webViewController: InAppBrowserWebViewController) {
    let storyboard = UIStoryboard(
      name: InAppBrowserManager.WEBVIEW_STORYBOARD, bundle: FlutterInAppWebViewIOSResources.bundle)
    let navController =
  ...
  public func openWithSystemBrowser(url: String, result: @escaping FlutterResult) {
    ...
    if !UIApplication.shared.canOpenURL(absoluteUrl) {
      result(
        FlutterError(code: "InAppBrowserManager", message: url + " cannot be opened!", details: nil)
      )
      return
    } else {
